...
> 첫 태그 릴리스는 해당 승인을 기록한 뒤 GitHub 저장소 변수 `DOCS_PUBLICATION_APPROVED=true`를 설정해야 진행됩니다. 자세한 내용은 `RELEASE_CHECKLIST.md`와 `NOTICE.md`를 확인하세요.
...
- 원본 레코드 순서를 지정하는 `table/fulltext order=asc|desc`
...
현재 저장소에는 오픈소스 라이선스가 지정되지 않았습니다. 코드 소유자의 승인을 받아 라이선스를 선택하기 전에는 사용·복제·배포 권한이 부여된 것으로 간주하지 마십시오.

...
프로세스 생존은 `GET /api/v1/health`, 문서 인덱스까지 준비됐는지는
`GET /api/v1/ready`로 확인합니다. `/ready`가 `503`이면 응답의 `failures`를
확인한 뒤 기준 문서를 배치하거나 재인덱싱하세요. 두 점검은 외부 Logpresso나
LLM 호출을 수행하지 않습니다.
...
- TLS 종료와 사용자 인증은 고객사의 리버스 프록시 또는 IdP에서 처리합니다.
- 프록시는 관리 API에 사용자 식별자만 `X-Actor-ID`로 전달할 수 있습니다.
  이 헤더를 인터넷 클라이언트가 직접 지정하지 못하게 프록시에서 제거하거나
  재설정하세요.
...
- `data/`는 백업하되, 카탈로그·피드백·감사 메타데이터만 포함하도록 접근 권한을
  최소화합니다. 고객 로그 원문이나 인증 정보는 이 폴더에 저장하지 않습니다.
